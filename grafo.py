...
class Grafo():
    ...
    def contar_compartidos(self):
        personajes = {}
        visitados = []
        aux = self.__inicio
        while aux is not None:
            visitados.append(aux.info)
            auxAdy = aux.adyacentes.get_inicio()
            while auxAdy is not None:
                if (auxAdy.info not in visitados):
                    personajes[f'{aux.info}-{auxAdy.info}'] = auxAdy.peso
                auxAdy = auxAdy.sig
            aux = aux.sig

        return personajes

    def contar_personajes_compartidos(self):
        personajes = {}
        visitados = []
        aux = self.__inicio
        while aux is not None:
            visitados.append(aux.info)
            auxAdy = aux.adyacentes.get_inicio()
            personajes[aux.info] = 0
            while auxAdy is not None:
                personajes[aux.info] += 1
                auxAdy = auxAdy.sig
            aux = aux.sig
